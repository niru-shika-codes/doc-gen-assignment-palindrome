...
def on_pre_process_complete(facts: dict) -> None:
    logger.info(
        "Pre-processing complete: client=%s, accounts=%d",
        facts["client_name"],
        len(facts["accounts"]),
    )
...
def on_investigation_complete(facts: dict) -> None:
    logger.info(
        "Investigation complete: disposal=%s, amount=%s, risk_profile=%s",
        facts.get("disposal"),
        facts.get("amount"),
        facts.get("risk_profile"),
    )
...
def on_validation_complete(issues: list) -> None:
    if issues:
        logger.warning(
            "Validation failed with %d issue(s) — review before sending",
            len(issues),
        )
        for issue in issues:
